...
export default function InventoryCategoriesPage() {
  ...
  useEffect(() => {
    if (!userLoading && (!user || !hasPermission("inventory.categories.read"))) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      if (!user || !hasPermission("inventory.categories.read")) return;
      setIsLoading(true);
      setError("");
      try {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (
      !confirm(
        `Are you sure you want to delete "${name}"? This will remove the category from all inventory items that use it.`
      )
    ) {
      return;
    }
  ...
  if (!user || !hasPermission("inventory.categories.read")) {
    return null;
  }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6 flex justify-between items-center">
        ...
        {hasPermission("inventory.categories.create") && (
          <Link
            href="/settings/inventory-categories/new"
                ...
                <tr key={category.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {hasPermission("inventory.categories.update") && (
                      <Link
                        href={`/settings/inventory-categories/${category.id}/edit`}
